...
def preprocessing(datasetFilePath):
    ...
    stopverbs = mIslemPlsa.dosyaOkuma("needlessTW.txt")
    from stop_words import get_stop_words
    stopwords = get_stop_words('turkish')

    # read the documents
    file = codecs.open(datasetFilePath, 'r')
    ...
    for document in documents:
        document = str(document).replace("[^a-zA-ZŞşÇçÖöİÜüığ#]", " ")
        segList = str(document).strip().split(' ')
        wordCount = {}
        for word in segList:
            word = word.lower().strip()
            if len(word) > 2 and word not in stopwords and word not in stopverbs:               
...
def output():
    ...
    file = codecs.open(topicWords,'w')
    for i in range(0, K):
        topicword = []
        ids = theta[i, :].argsort()
        for j in ids:
            topicword.insert(0, id2word[j] + '-' + str(theta[i,j]))
        tmp = ''
        for word in topicword[0:min(topicWordsNum, len(topicword))+10]:
            tmp += word + ' '
        file.write(tmp + '\n')
    file.close()

#Tweet için Duygu5C , haber için HaberlerLsa5C
datasetFilePath = 'Duygu5C.txt'
K = 20    # number of topic
maxIteration = 10
threshold = 15.0
topicWordsNum = 40
topicWords = 'pLSAtopics5C_.txt'
if(len(sys.argv) == 11):
